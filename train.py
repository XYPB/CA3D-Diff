...
def load_pretrain_stable_diffusion(new_model, finetune_from):
    ...
    # 统计信息
    load_ratio = len(matched_state) / len(new_state)
    print(f"Matched keys: {len(matched_state)}/{len(new_state)} ({load_ratio:.1%})")
...
if __name__ == "__main__":
    ...
    trainer = Trainer.from_argparse_args(args=argparse.Namespace(), **trainer_config,
                                         accelerator='gpu', strategy=DDPStrategy(find_unused_parameters=False), logger=logger, callbacks=callbacks)
