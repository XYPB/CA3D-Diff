...
from ldm.modules.attention import SpatialTransformer,DualInputSpatialTransformer
    ...
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, DualInputSpatialTransformer):
                x = layer(x, context)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else DualInputSpatialTransformer(  # always uses a self-attn
                            ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,column_decay=True
                        ),
            ResBlock(
                ch,
                time_embed_dim,
    ...
    def _select_context(self, h, context_dict):
        current_res = h.shape[2]
        return context_dict[str(current_res)]
